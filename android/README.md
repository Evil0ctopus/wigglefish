...
4. Tap **Identify chip**. The app:
   ...
   - Sends ROM `SYNC`, then prefers `GET_SECURITY_INFO` chip_id (C3=`5`, C5=`23`, â€¦)
...
| Exit | `FLASH_END` + hard reset | Leave download mode â†’ run app |
...
~1.3 MB total. Keep the OTG cable still for 1â€“3 minutes. Desktop ESP Web Tools / `idf.py flash` remain valid alternatives when the draft release is published.
...
To build a signed ssembleRelease APK (for GitHub Releases):

1. Create a dedicated keystore outside the repo, e.g. %USERPROFILE%\.wigglefish\wigglefish-upload.jks.
2. Create ndroid/keystore.properties (gitignored) with storeFile, storePassword, keyAlias, keyPassword.
3. From ndroid/: .\gradlew.bat assembleRelease
4. APK lands at pp/build/outputs/apk/release/app-release.apk.
